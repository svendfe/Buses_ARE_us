...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation)  {
        ...
        Iterator<Stop> n=StopManager.getInstance().iterator();
        super.updateVisibleArea();
        this.newStopClusterer();
        while(n.hasNext()){
            Stop s=n.next();
            Marker m = new Marker(super.mapView);
            this.setMarker(s, m);
            if(Geometry.rectangleContainsPoint(super.northWest,super.southEast,s.getLocn())) {
                m.setPosition(new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude()));
                m.setIcon(stopIconDrawable);
                m.setTitle("Stop number: "+s.getNumber()+"\nStop Name: "+ s.getName());
        ...
        // task 5 plot stops and call method to update nearest
    }
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if(this.stopMarkerMap.size()!=0){

        Marker m = this.nearestStnMarker;
        if (nearest != null) {
            if (m != null) {
                m.setIcon(stopIconDrawable);
                m= null;
            }
            m = this.getMarker(nearest);
        ...
        nearestStnMarker = m;
        if(nearest!=null) {
            BusesAreUs.activity.onLocationChanged(nearest, nearest.getLocn());
        }else{
            BusesAreUs.activity.onLocationChanged(null,new LatLon(0,0));
        }

        }


       //task 6:update closest marker
    }
